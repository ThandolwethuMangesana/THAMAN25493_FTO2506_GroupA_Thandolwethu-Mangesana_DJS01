export class Modal {
  ...
  _bindEvents() {
    ...
    window.addEventListener('click', e => {
      if (this.modal && e.target === this.modal) this.close();
    });
    
    if (!Modal._escapeListenerAdded) {
      window.addEventListener('keydown', e => {
        if (e.key === 'Escape') {
          // Close all open modals (assuming only one modal is open at a time)
          document.querySelectorAll('.modal:not(.hidden)').forEach(modalEl => {
            modalEl.classList.add('hidden');
          });
        }
      });
      Modal._escapeListenerAdded = true;
    }
  }
